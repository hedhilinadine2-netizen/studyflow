...
$stmt = $pdo->prepare("SELECT * FROM classes WHERE id = ? AND user_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($class_name) || empty($subject)) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE classes SET class_name = ?, subject = ?, teacher = ?, room = ?, day_of_week = ?, start_time = ?, end_time = ? WHERE id = ?");
            $stmt->execute([$class_name, $subject, $teacher, $room, $day_of_week, $start_time, $end_time, $class_id]);
            header("Location: classes.php");
            exit;
...
<head>
    ...
    <title>Edit Class - StudyFlow</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-group input, .form-group select { width: 100%; padding: 12px 16px; border: 1px solid #e8ecf2; border-radius: 12px; font-size: 0.9rem; }
...
<aside class="sidebar">
    ...
    <nav class="sidebar-menu">
        ...
        <a href="classes.php" class="s-link active">Classes</a>
...
<main class="main">
    ...
    <div class="form-container">
        <h1 class="form-title">Edit Class</h1>
        <div class="form-subtitle">Update class details</div>
        ...
        <form method="POST" action="">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select name="subject" required>
                        <option value="Mathematics" <?php echo $class['subject'] == 'Mathematics' ? 'selected' : ''; ?>>Mathematics</option>
                        <option value="Physics" <?php echo $class['subject'] == 'Physics' ? 'selected' : ''; ?>>Physics</option>
                        <option value="PHP" <?php echo $class['subject'] == 'PHP' ? 'selected' : ''; ?>>PHP</option>
                        <option value="Web Development" <?php echo $class['subject'] == 'Web Development' ? 'selected' : ''; ?>>Web Development</option>
                        <option value="Database" <?php echo $class['subject'] == 'Database' ? 'selected' : ''; ?>>Database</option>
                        <option value="English" <?php echo $class['subject'] == 'English' ? 'selected' : ''; ?>>English</option>
                        <option value="Computer Science" <?php echo $class['subject'] == 'Computer Science' ? 'selected' : ''; ?>>Computer Science</option>
                    </select>
                </div>
                <div class="form-group">
                    <label>Teacher</label>
                    <input type="text" name="teacher" value="<?php echo htmlspecialchars($class['teacher'] ?? ''); ?>">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    <label>Room</label>
                    <input type="text" name="room" value="<?php echo htmlspecialchars($class['room'] ?? ''); ?>">
                </div>
                <div class="form-group">
                    <label>Day</label>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="time" name="start_time" value="<?php echo $class['start_time'] ?? ''; ?>">
                </div>
                <div class="form-group">
                    <label>End Time</label>
                    <input type="time" name="end_time" value="<?php echo $class['end_time'] ?? ''; ?>">
                </div>
            </div>
