...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($class_name) || empty($subject)) {
        ...
        $stmt = $pdo->prepare("INSERT INTO classes (user_id, class_name, subject, teacher, room, day_of_week, start_time, end_time, color, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active')");
        
        if ($stmt->execute([$_SESSION['user_id'], $class_name, $subject, $teacher, $room, $day_of_week, $start_time, $end_time, $color])) {
            $success = "Class added successfully!";
            echo "<script>setTimeout(function(){ window.location.href = 'classes.php'; }, 1500);</script>";
        } else {
...
<head>
    ...
    <title>StudyFlow - Add Class</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        <form method="POST" action="">
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="class_name" placeholder="e.g., Advanced PHP" required>
                </div>
                <div class="form-group">
            ...
            <div class="form-row">
                <div class="form-group">
                    <label>Teacher</label>
                    <input type="text" name="teacher" placeholder="e.g., Dr. Smith">
                </div>
                <div class="form-group">
                    <label>Room</label>
